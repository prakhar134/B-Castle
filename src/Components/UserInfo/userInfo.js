...
const UserInfo = (props) => {
  ...
  if (!user) return <span className="loading">Loading...</span>;
  ...
  if (props.update)
    return (
      <>
        <h1 style={{ textAlign: "center", maxWidth: "80%" }}>
          {msg} of {user?.user.name.split(" ")[0]}
        </h1>
        <h3
          style={{
            color: "#555",
        ...
        >
          <div style={{ marginBottom: "25px" }} className="input_group">
            ...
            <h2 style={{ margin: "-2px", fontSize: "2rem" }}>
              ₹{user?.user.balance}
            </h2>
          </div>
  ...
  return (
    <div>
      <h1>Welcome Aboard, {user?.user?.name?.split(" ")[0]}</h1>
      <h3
        style={{
          color: "#555",
          marginTop: "-15px",
